...
export function ContactForm() {
  ...
  const schema = useMemo(
    () =>
      z.object({
        ...
        consent: z.literal(true, { errorMap: () => ({ message: t("errors.consentRequired") }) }),
      }),
    [t],
  );
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) {
      setFileError(t("errors.fileTypeUnsupported"));
      setSelectedFile(null);
      e.target.value = "";
      return;
    }
  ...
  if (status === "ok") {
    return (
      <div className="border border-[color:var(--color-gold)] bg-[color:var(--color-bone-soft)] p-8">
        <p className="eyebrow text-[color:var(--color-gold-dim)]">{t("successEyebrow")}</p>
        <h3 className="mt-3">{t("successTitle")}</h3>
        <p className="mt-4 text-[color:var(--color-ink)]/80">
          {t("successBody")} <strong>800 080 000</strong> {t("successFreeCall")}
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div>
        ...
        <p className="mt-1.5 text-xs text-[color:var(--color-stone-dark)]">{t("fileHint")}</p>
      </div>
      ...
      {status === "error" && (
        <p className="text-sm text-[color:var(--color-danger)]">{t("errorGeneric")}</p>
      )}
    </form>
  );
}
