...
export function PropertyCard({ slug, frontmatter }: PropertyCardProps) {
  const tCard = useTranslations("imobiliario.card");
      ...
      <div className="relative aspect-[4/3] overflow-hidden bg-[color:var(--color-stone)]/20">
        {frontmatter.hero && (
          <>
            ...
            <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
              <Image src="/Logos/CNRC/Logos_CNRC_Transparent.png" alt="" width={60} height={24} className="opacity-25" style={{ objectFit: "contain" }} />
            </div>
          </>
        )}
        ...
        {frontmatter.energyRating && (
          <div className="absolute bottom-3 right-3 bg-white/95 backdrop-blur-sm px-2.5 py-1.5 shadow-lg z-10">
            <p className="text-[9px] uppercase tracking-wider text-[color:var(--color-stone-dark)] font-medium">{tCard("energyClass")}</p>
            <p className="text-xl font-bold text-[color:var(--color-navy)] leading-none mt-0.5">{frontmatter.energyRating}</p>
          </div>
        )}
      </div>

      <div className="p-4">
        <p className="text-xs text-[color:var(--color-stone-dark)] uppercase tracking-wider">{frontmatter.location}</p>
        ...
        <div className="mt-3">
          {frontmatter.price ? (
            <span className="text-base font-semibold text-[color:var(--color-gold)]">{frontmatter.price}</span>
          ) : (
            <span className="text-sm text-[color:var(--color-stone-dark)]">{tCard("onRequest")}</span>
          )}
        </div>
      </div>
    </Link>
